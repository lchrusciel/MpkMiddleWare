...
class StopAdapterSpec extends ObjectBehavior
{
    ...
    )
    {
        ...
        $stopFactory->create(array('repository1'))->willReturn($stop1);
        $stopFactory->create(array('repository2'))->willReturn($stop2);
    ...
    function it_provides_stops_for_given_line(
        $stopRepository,
        $stopFactory,
        stop $stop1,
        stop $stop2
    ...
    {
        $stopRepository->getStopsForLine(1)->willReturn(array(array('repository1'),array('repository2')));

        $stopFactory->create(array('repository1'))->willReturn($stop1);
        $stopFactory->create(array('repository2'))->willReturn($stop2);

        $this->getStopsForLine(1)->shouldReturn(array($stop1,$stop2));
    }
}